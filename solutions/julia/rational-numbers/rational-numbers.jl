...
struct RationalNumber{T <: Integer} <: Real
    ...
    RationalNumber{T}(n::T) where T <: Integer = RationalNumber{T}(n, one(T))

end

RationalNumber(n::T, d::T) where T <: Integer = RationalNumber{T}(n, d)
RationalNumber(n::T) where T <: Integer = RationalNumber{T}(n, one(T))
...
^(r::RationalNumber, n::Integer) = n >= 0 ? RationalNumber(r.n ^ n, r.d ^ n) : RationalNumber(r.d ^ n, r.n ^ n)
^(x::Real, r::RationalNumber) = x ^ (r.n / r.d)
