...
namespace space_age {
    ...
    space_age::space_age(const uint64_t &seconds) : m_seconds(seconds) {}

    uint64_t space_age::seconds()    const { return m_seconds; }
    double   space_age::on_earth()   const { return seconds() / (365.25 * 86400); }
